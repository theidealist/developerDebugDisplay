...
int main(int argc, char* argv[])
{
    ...
    d3::di().add( "first::line",
                   d3::get(d3::Line{osg::Vec3(0,0,0),
                                    osg::Vec3(1,1,1),
                                    d3::nextColor()}) );
    d3::di().add( "first::dot",
                   d3::get(d3::Point{osg::Vec3(1,0,0), d3::nextColor()}) );


    d3::di().add( "first::second::deep",
                   d3::get(d3::Point{osg::Vec3(1,1,0), d3::nextColor()}) );

    d3::di().add( "first::cylinder",
                   d3::get(d3::Cylinder{osg::Vec3(3,3,3), osg::Vec3(5,5,5), 0.2, osg::Vec4(0,1,1,1)}) );

    d3::di().add( "first::capsule",
                   d3::get(d3::Capsule{osg::Vec3(-5,-5,5), osg::Vec3(-3,-3,3), 0.3, osg::Vec4(1,1,0,1)}) );

    d3::di().add( "first::sphere",
                   d3::get(d3::Sphere{osg::Vec3(1,2,3), 0.1, osg::Vec4(0.2, 0.3, 0.4, 0.5)}) );
    ...
    d3::di().add( "par::not::appender", d3::get(d3::Point{osg::Vec3(2,0,0), d3::white()}), replace);
    d3::di().add( "par::not::appender", d3::get(d3::Point{osg::Vec3(3,0,0), d3::white()}), replace);

    replace = false;
    d3::di().add( "par::appender", d3::get(d3::Point{osg::Vec3(4,0,0), d3::white()}), replace);
    d3::di().add( "par::appender", d3::get(d3::Point{osg::Vec3(5,0,0), d3::white()}), replace);
    d3::di().add( "par::appender", d3::get(d3::Point{osg::Vec3(6,0,0), d3::white()}), replace);
    ...
    MT->setMatrix(osg::Matrix::translate(osg::Vec3(0.0, 0.0, 3.0)));
    ...
    d3::di().add( "first::pose",
                   d3::get(d3::Triad{osg::Matrix::translate(osg::Vec3(0.2, 0.3, -0.1))}) );

    // try making a mesh
    d3::PointVec_t points;
    for ( double xx(-1.0) ; xx<=1.0 ; xx+=0.01 )
        for ( double yy(-2.0) ; yy<=2.0 ; yy+=0.01 )
            points.push_back(d3::Point{osg::Vec3(xx,yy,0.1), osg::Vec4(1,0,1,0.3)});
    uint width( 4/0.01 );
    d3::di().add( "mesh::filled", d3::get(d3::MeshGrid{points, std::vector<osg::Vec3>(1, {0,0,1}), width, true}) );
    d3::di().add( "mesh::wireframe", d3::get(d3::MeshGrid{points, std::vector<osg::Vec3>(1, {0,0,1}), width, false}) );
    d3::di().add( "mesh::height", d3::get(d3::HeightGrid{points, 0.01, 0.01, osg::Vec3(-1.0, -2.0, 0.0), width, osg::Vec4(0,1,1,0.5)}) );

    osg::ref_ptr<osg::Node> point( d3::get(d3::Point{osg::Vec3(0,0,0), d3::nextColor()}) );
    ...
    if ( root )
    {
        ...
        osg::ref_ptr<osg::MatrixTransform>
            scaleXform(new osg::MatrixTransform(osg::Matrix::scale(osg::Vec3(1.0,1.0,0.1))));
